...
- Edge Connect - Manages the connection and management of offchain workers (K3S clusters).
- Task Management - Manages scheduling of tasks into remote K3s clusters based on availability and specifications.

This prototype is a blockchain-based ML training module with on-chain verification and settlement. It involves more than two virtual machines to accurately recreate the real-world scenario of the blockchain and the off-chain components (K3S clusters). These components interact with the blockchain to execute tasks, submit results, and update task execution status in a coordinated manner.

To simplify the process of testing the setup of K3S workers and their connection to the Substrate blockchain, we have a pre-hosted version of the Cyborg parachain on the Tanssi Dancebox testnet. This setup includes an active K3S cluster, allowing you to directly test the application's workflow. We will discuss both local and hosted methods of testing to provide better clarity regarding the delivery.
...
The edge-connect pallet provides functionalities to manage connected workers within the system. It offers functionality to register (`register_worker`) and remove (`remove_worker`) workers associated with user accounts. A storage map tracks worker details including status, IP, domain, availability status, specs, and creation block.

This information is then used by the task-management pallet to issue tasks to connected workers.

A task can be submitted by any account using the `task_scheduler` extrinsic. The task is then assigned to an account with a registered worker, known as the `executor`. Once the task is completed, the executor submits an extrinsic call, (`submit_completed_task`), which includes the assigned `TaskId` and the hash output of the task.

This information is then used to verify the task by another account with a registered worker, designated as the `verifier`. The verifier performs the task and submits their own hash output along with the `TaskId` using the `verify_completed_task` extrinsic. If the hashes match, the task is marked as `complete`. If the hashes do not match, a different account with a registered worker is assigned the role of `resolver`.

The resolver performs the task and submits a hash output with the same `TaskId` using the `resolve_completed_task` extrinsic. The hash that matches either the `executor` or the `verifier` is considered the correct task output. If the resolver also fails to produce a matching hash, the task is `reassigned` to a new executor, different from all three previously involved accounts in the task verification process.
...
Only accounts with a minimum balance can execute transactions. The Alice account is prefunded with the chain's tokens.
If you are testing either locally or using the hosted version of our chain, you will need to use the Alice account. This wallet will be used to interact with the frontend as well.
...
Here is an example of adding the Alice account to the Polkadot.js wallet extension using the seed phrase:
...
7) Node List screen. There should already be a worker registered on-chain. The tasks are assigned randomly to available workers.
If you registered another worker, the task might be executed by another worker.
Just click through the workers by clicking the left `Dashboard` tab to go back and see the other workers at this time to check the executed task.
...
We require a local Linux machine and two servers hosted within the same virtual network (such as a VPC in the case of AWS)

- Local machine: Parachain and Frontend.
...
It is crucial that both the K3S master node and worker node are connected within the same network. This setup is essential to ensuring seamless load distribution, especially when managing heavier data loads, such as ML models.
...
5. let's cross-verify the VNet of both deployed servers.
...
<img width="1000" alt="inbound rule setup" src="assets/worker/edit-inbound-port-rule.png"><br><br>
...
You should now see the new rule within you `Network Settings` dashboard.

<img width="400" alt="inbound rule setup" src="assets/worker/add-inbound-rule.png"><br><br>
...
Once the `master node` receives instructions from the blockchain, it assigns the tasks to its `worker nodes` for execute. Due to the networking requirements, the `master node` and `worker nodes` should be set up on separate machines, ideally within the same local network.
...
Checkout the branch for the parachain:
...
After setting up the master node, add worker nodes to the cluster using the join token from `k3s-node-token.txt`.
On another machine within the same network as the master node, clone the same repository. You do not need to run `npm install` or execute `MasterSetup.sh`.
keep track of the local IP address of your master node. You will need both the local IP address and the join token from `k3s-node-token.txt` to setup the worker.
...
In the master node dashboard, retrieve the IP of the master node, and use it in the command:
...
The private IP address in this example is `10.0.0.5`.
