use crate::mock::*;
use crate::{
    pallet::Config, SubmittedPerPeriod, WorkerStatusEntriesPerPeriod, StatusInstance, LastClearedBlock, ResultingWorkerStatus, ResultingWorkerStatusPercentages, ProcessStatusPercentages };
use frame_support::{sp_runtime::RuntimeDebug, BoundedVec, testing_prelude::bounded_vec, pallet_prelude::ConstU32, assert_storage_noop, assert_noop };
...
fn on_new_data_works_as_expected() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            WorkerStatusEntriesPerPeriod::<Test>::get(&key_1),
            vec![],
        );
        ...
        let mut entries: BoundedVec<StatusInstance<BlockNumberFor<Test>>, <Test as Config>::MaxAggregateParamLength> = BoundedVec::try_from(vec![resulting_status_instance_1]).unwrap();
        ...
        StatusAggregator::on_new_data(&oracle_feeder_1, &key_1, &ProcessStatus {
            online: false,
            available: false,
        });
        ...
        entries.try_push(resulting_status_instance_2.clone()).unwrap();
        ...
        let entries_2: BoundedVec<StatusInstance<BlockNumberFor<Test>>, <Test as Config>::MaxAggregateParamLength> = BoundedVec::try_from(vec![resulting_status_instance_2]).unwrap();
        let entries_3: BoundedVec<StatusInstance<BlockNumberFor<Test>>, <Test as Config>::MaxAggregateParamLength> = BoundedVec::try_from(vec![resulting_status_instance_3]).unwrap();

        // assert correct state changes
        assert_eq!(
            WorkerStatusEntriesPerPeriod::<Test>::get(&key_2),
            entries_2,
        );
        assert_eq!(
            WorkerStatusEntriesPerPeriod::<Test>::get(&key_3),
            entries_3,
        );
    });
}
...
fn on_finalize_works_as_expected() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            WorkerStatusEntriesPerPeriod::<Test>::iter().collect::<Vec<_>>().len(),
            0,
        );
        // initalize test variables
        let inital_block = 1;
        ...
        StatusAggregator::on_new_data(&oracle_feeder_2, &key_1, &ProcessStatus {
            online: false,
            available: true,
        });
        StatusAggregator::on_new_data(&oracle_feeder_1, &key_2, &ProcessStatus {
            online: true,
            available: false,
        });
        StatusAggregator::on_new_data(&oracle_feeder_2, &key_2, &ProcessStatus {
            online: false,
            available: false,
        });
        ...
        assert!(
            SubmittedPerPeriod::<Test>::iter().collect::<Vec<_>>().len() > 0
        );
        assert!(
            WorkerStatusEntriesPerPeriod::<Test>::iter().collect::<Vec<_>>().len() > 0
        );
