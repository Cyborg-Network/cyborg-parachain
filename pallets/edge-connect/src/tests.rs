...
fn it_works_for_inserting_miner_into_correct_storage() {
    ...
    assert_ok!(EdgeConnectModule::add_account_authorized_for_registration(
		...
		// Dispatch a signed extrinsic for miner 0
    ...
    assert_ok!(EdgeConnectModule::add_account_authorized_for_registration(
		...
		// Dispatch a signed extrinsic for miner 1
		assert_ok!(EdgeConnectModule::register_miner(
		...
		// Read pallet storage and assert an expected result.
		assert_eq!(pallet_edge_connect::EdgeMiners::<Test>::get(bounded_uuid_edge), Some(miner_1));
	});
}
...
fn it_fails_for_registering_duplicate_miner() {
    ...
    assert_ok!(EdgeConnectModule::add_account_authorized_for_registration(
		...
		assert_noop!(
			...
			Error::<Test>::CanOnlyRegisterOneMinerPerAccount
		);
	});
}
...
fn it_works_for_removing_miner() {
    ...
    assert_ok!(EdgeConnectModule::add_account_authorized_for_registration(
		...
		assert_eq!(pallet_edge_connect::CloudMiners::<Test>::get(bounded_uuid_cloud.clone()), None);
		// Assert that the miner no longer exists
		assert_eq!(pallet_edge_connect::EdgeMiners::<Test>::get(bounded_uuid_edge.clone()), None);
	});
}
...
#[test]
fn emitting_proper_event_for_registering_miner() {
	new_test_ext().execute_with(|| {
		let alice = 0;
